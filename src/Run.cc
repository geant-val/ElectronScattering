...
void Run::EndOfRun()
{
  ...
  std::cout << "\n The run was " << numberOfEvent << " " << partName << " of "
         ...
         << G4BestUnit(density,"Volumic Mass") << ")" << std::endl;;
...
void Run::PrintFluence(G4int TotEvents)
{
  ...
  File << "  Fluence density distribution \n " 
       << "\n  ibin \t angle (deg) \t radius (mm) \t Nb \t fluence\t norma fl\t rms/nfl (%) \n"
       << std::endl;;
  ...
  for (G4int bin=0; bin<fNbBins; bin++) {
     rmean +=fDr;
     G4double error = 0.;
     angle = std::atan(rmean/mm/1182.0);
     if (fluence1[bin] > 0.) error =  100*fluence2[bin]/fluence1[bin];
     File << "  " << bin << "\t " << angle/deg << "\t " << rmean/mm << "\t " << fNbEntries[bin]
