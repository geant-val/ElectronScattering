...
{ 
  ...
  /*
  G4Tubs*
  scorer = new G4Tubs("scoringvolume",                        //name
                         0*cm, fRadius_World,             //r1, r2
                         0.5*mm,            //half-length 
                         0., twopi);                      //theta1, theta2
                         
  G4LogicalVolume*                            
  lscorer = new G4LogicalVolume(scorer,            //its solid
                                   //fMaterial_World,       //its material
                                   G4NistManager::Instance()->FindOrBuildMaterial("G4_WATER"), 
                                   "scoringvolume");              //its name
                                   
  zpos = 0.5*fThickness_World + 1*mm; 
                                     
  new G4PVPlacement(0,                      //no rotation
                        G4ThreeVector(0,0,zpos),          //translation
                                 lscorer,              //its logical volume
                                 "scorervolume",                 //its name
                                 lvol_World,              //its mother  volume
                                 false,                   //no boolean operation
                                 0);                      //copy number

   G4Region* scoringregion = G4RegionStore::GetInstance()->FindOrCreateRegion("scoringregion");
   lscorer->SetRegion(scoringregion);
   scoringregion->AddRootLogicalVolume(lscorer);
  */                                            
  // ExitWindow
  //
  G4Tubs*
...
void DetectorConstruction::SetMaterialExitWindow(G4String material)
{
  // search the material by its name
  G4Material* pMaterial = G4Material::GetMaterial(material);

  if (pMaterial) fMaterial_ExitWindow = pMaterial;                  
}
