...
{ 
  ...
  fMaterCmd = new G4UIcmdWithAString("/testem/det/setScatteringFoilMaterial",this);
  fMaterCmd->SetGuidance("Select Material of the Scatter Foil");
  fMaterCmd->SetParameterName("material",false);

  fMaterEWCmd = new G4UIcmdWithAString("/testem/det/setExitWindowMaterial",this);
  fMaterEWCmd->SetGuidance("Select Material of the exit window");
  fMaterEWCmd->SetParameterName("material",false);
...
{ 
  ...
  if ( command == fMaterEWCmd )
   {fDetector->SetMaterialExitWindow(newValue);}
